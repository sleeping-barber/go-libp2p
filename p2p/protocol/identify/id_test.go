...
func subtestIDService(t *testing.T) {
	h1, err := base.NewHost(swarmt.GenSwarm(t))
	require.NoError(t, err)
	defer h1.Close()

	h2, err := base.NewHost(swarmt.GenSwarm(t))
	require.NoError(t, err)
	defer h2.Close()
...
func TestLargeIdentifyMessage(t *testing.T) {
	...
	h1, err := base.NewHost(swarmt.GenSwarm(t, swarmt.OptPeerPrivateKey(sk1)))
	require.NoError(t, err)
	defer h1.Close()

	h2, err := base.NewHost(swarmt.GenSwarm(t, swarmt.OptPeerPrivateKey(sk2)))
	require.NoError(t, err)
	defer h2.Close()
